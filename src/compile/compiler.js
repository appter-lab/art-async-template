...
class Compiler {
    ...
    constructor(options) {
        ...
        this.internal = {
            ...
            [BLOCKS]: `arguments[1]||{}`,
            [FROM]: `null`,
            [PRINT]: `async function(){var s=''.concat.apply('',arguments);${OUT}+=s;return s}`,
            [INCLUDE]: `async function(src,data){var s=await ${OPTIONS}.include(src,data||${DATA},arguments[2]||${BLOCKS},${OPTIONS});${OUT}+=s;return s}`,
            [EXTEND]: `async function(from){${FROM}=from}`,
            [SLICE]: `async function(c,p,s){p=${OUT};${OUT}='';await c();s=${OUT};${OUT}=p+s;return s}`,
            [BLOCK]: `async function(){var a=arguments,s;if(typeof a[0]==='function'){return await ${SLICE}(a[0])}else if(${FROM}){if(!${BLOCKS}[a[0]]){${BLOCKS}[a[0]]=await ${SLICE}(a[1])}else{${OUT}+=${BLOCKS}[a[0]]}}else{s=${BLOCKS}[a[0]];if(typeof s==='string'){${OUT}+=s}else{s=await ${SLICE}(a[1])}return s}}`
        };
